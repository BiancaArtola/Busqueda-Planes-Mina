...
% Rejas:
%% Reja = [r; NombreR], siendo NombreR el nombre asociado a la reja.
estaEn([r, r1], [13,8]).
estaEn([r, r2], [5,2]).
estaEn([r, r3], [7,10]).

% Llaves:
% Llave = [l; NombreL], siendo NombreL el nombre asociado a la llave;
estaEn([l, l1], [9,6]).
estaEn([l, l2], [5,8]).
estaEn([l, l3], [12,9]).

% Carga:
% Carga = [c; NombreC], siendo NombreC el nombre asociado a la carga;
estaEn([c, c1], [6,6]).

% Detonador:
% Detonador = [d; NombreD; ActivadoD], siendo NombreD el nombre asociado al detonador,
% y ActivadoD 2 fsi; nog un indicador que establece si el detonador se encuentra activado o no
estaEn([d, d1, no], [6,2]).
